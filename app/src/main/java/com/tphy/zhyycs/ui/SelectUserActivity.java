...
//        GetAllUser();
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        try {
                            ...
                            if (success.equals("true")) {
                                ...
                                adapter = new SelectUserAdapter(SelectUserActivity.this, list_user);
    ...
    private void initView() {
        context = SelectUserActivity.this;
    ...
    private void GetAllUser() {
        ...
        pd.show();
        Map<String, String> params = new HashMap<>();
        params.put("paraJson", Common.params("codeTables", "projectUser"));
        String url = DemoApplication.serviceUrl + "/GetCodeListByName";
        OkHttpUtils.post()
                .url(url)
                .params(params)
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        try {
                            ...
                            if (success.equals("true")) {
                                ...
                                result = jsonObject.getString("result");
                                JSONObject js = new JSONObject(result);
                                List<User> list = new Gson().fromJson(js.getString("projectUser"), new TypeToken<List<User>>() {
                                }.getType());

                                Log.e("数组的长度", list.size() + "");
                                for (int i = 0; i < list.size(); i++) {
                                    if (list.get(i).getDutyCode().equals(number)) {
                                        list_user.add(list.get(i));
                                        Log.e("添加的数据", list.get(i).getName());
                                    }
                                }
        ...
        if (!("".equals(type))) {
            if (type.equals("部门")) {
                ...
                if (list_bumen.size() > 0) {
                    ...
                    Toast.makeText(SelectUserActivity.this, "请选择部门", Toast.LENGTH_SHORT).show();
                }
            }
            if (list_add.size() > 1) {
                switch (type) {
                    case "审核人":
                        Toast.makeText(SelectUserActivity.this, type + mToast, Toast.LENGTH_SHORT).show();
                        break;
                    case "抄送人":
                        Toast.makeText(SelectUserActivity.this, type + mToast, Toast.LENGTH_SHORT).show();
                        break;
    ...
    private TextWatcher watcher = new TextWatcher() {
        ...
        public void afterTextChanged(Editable s) {
            if (s.length() > 0) {
                ...
                adapter = new SelectUserAdapter(SelectUserActivity.this, list);
                listview.setAdapter(adapter);
            } else {
